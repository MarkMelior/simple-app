...
Запустим команду для инициализации Prisma в внашем проекте:

```bash
npx prisma init
```

Это создаст директорию `prisma` с файлом `schema.prisma` и файл `.env`

Откроем файл `prisma/schema.prisma` и определим модель данных. Например, создадим модель User:
...
```typescript filename="jwt.ts" githubPath="src/features/auth/api/jwt.ts"
...
```typescript filename="session.ts" githubPath="src/features/auth/api/session.ts"
import 'server-only';
...
```typescript filename="definitions.ts" githubPath="src/features/auth/types/definitions.ts"
import { z } from 'zod';
